      ...
      for (const healthKey of healthKeys) {
        ...
        if (healthData.quarantined === 'true') {
          ...
          if (healthData.quarantineUntil && new Date() > new Date(healthData.quarantineUntil)) {
            logger.info(`⏰ Quarantine period expired for account ${accountId}, attempting recovery`)
  ...
  async attemptAccountRecovery(accountId, accountType) {
    try {
      ...
      if (Date.now() < nextAttemptTime.getTime()) {
        logger.debug(`⏰ Too early for recovery attempt on ${accountId}, next attempt at ${nextAttemptTime}`)
        return false
      }

      logger.info(`🔄 Attempting recovery for account ${accountId} (attempt ${attempts + 1}/${this.MAX_RECOVERY_ATTEMPTS})`)
        ...
        if (sessionData) {
          try {
            ...
            if (mapping.source === 'fallback' && mapping.originalGroupId) {
              ...
              if (migrated) {
                migrationAttempts++
                logger.success(`🔄 Migrated session ${sessionHash} back to group ${mapping.originalGroupId}`)
              }
            }
          } catch (e) {
            // 忽略解析错误
  ...
  async triggerSessionMigration(accountId, accountType) {
    try {
      ...
      for (const groupId of affectedGroups) {
        logger.info(`🎯 Triggering session migration for group ${groupId} after account ${accountId} recovery`)
        
        // 记录迁移触发
        const migrationKey = `${this.SESSION_MIGRATION_PREFIX}${groupId}`
        await client.setex(migrationKey, 3600, JSON.stringify({
        ...
        if (sessionData) {
          try {
            ...
            if (mapping.source === 'fallback' && 
                affectedGroups.includes(mapping.originalGroupId)) {
              return true
            }
          } catch (e) {
            // 忽略解析错误
  ...
  async cleanupExpiredRecoveryData() {
    try {
      ...
      if (cleanedAttempts > 0 || cleanedMigrations > 0) {
        logger.info(`🧹 Cleaned up ${cleanedAttempts} expired recovery attempts and ${cleanedMigrations} migration records`)
      }
    } catch (error) {
